...
class Mastodon:
    ...
    # 追加分 fav
    ###
    def fav(self, max_id = None, since_id = None, limit = None):
        # def __api_request(self, method, endpoint, params = {}, files = {}, do_ratelimiting = True, header = None):
        """
        Fetch the authenticated user's favourited statuses.
        ...
        params = self.__generate_params(locals())
        header = 1
        return self.__api_request('GET', '/api/v1/favourites', params, None, None, header)
    ...
    def __api_request(self, method, endpoint, params = {}, files = {}, do_ratelimiting = True, header = None):
        ...
        while not request_complete:
            ...
            try:
                if header == 1:
                    maxid = re.search('\d{7,}',response_object.headers['link']).group(0)
                    max_id = str(maxid)
                    max_id = {'max_id': max_id}
                    response_a = response_object.json()
                    response_a.append(max_id)
                    response = response_a
                else:
